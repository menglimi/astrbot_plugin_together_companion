...
class TogetherCompanionPlugin(Star):
    ...
    async def _open_room_from_llm_tool(self, mode: str, event: AstrMessageEvent) -> str:
        ...
        if not self.server_enabled:
            return "共同房间服务未启用，无法打开房间。"
        if not self.room_server.running:
            try:
                await self.room_server.start()
            except Exception as exc:
                return f"共同房间服务启动失败：{_single_line(exc)}"

        normalized_mode = normalize_room_mode(mode)
        sender_id = _single_line(event.get_sender_id(), 80)
        ticket = self.issue_room_ticket(mode=normalized_mode, user_id=sender_id)
        room_url = self._ticket_url(ticket)
        browser_opened = False
        try:
            browser_opened = bool(
            ...
            logger.warning(
                "[TogetherCompanion] LLM 工具唤起系统浏览器失败: mode=%s error=%s",
                normalized_mode,
                _single_line(exc),
            )
        ...
        if browser_opened:
            browser_note = "已在运行 AstrBot 的电脑上尝试打开系统默认浏览器。"
        else:
            browser_note = "未能自动唤起浏览器，请使用下面的链接打开。远程部署时浏览器通常只能在 AstrBot 所在电脑上打开。"
        selection_note = (
            "页面打开后，请由用户选择或粘贴要看的视频；当前工具不会自动选择或播放视频。"
            if normalized_mode == "watch"
            else "页面打开后即可开始文字或语音通话。"
        )
        return f"{label}房间已准备好。\n{browser_note}\n{selection_note}\n房间链接：{room_url}"
    ...
    async def open_together_watch_room_tool(self, event: AstrMessageEvent) -> str:
        ...
        3. 工具只负责准备房间和打开页面，不代表已经选定或开始播放任何视频。
        4. 工具返回的链接必须交给用户；页面中的视频由用户最终选择。
        """
    ...
    async def open_together_call_room_tool(self, event: AstrMessageEvent) -> str:
        ...
        3. 工具只负责准备房间和打开页面，不要声称通话已经接通；用户打开页面后才会连接。
        """
    ...
    async def together_command(self, event: AstrMessageEvent):
        ...
        if not self.room_server.running:
            yield event.plain_result("实时房间服务尚未启动，请检查插件配置和启动日志。")
            return
        sender_id = _single_line(event.get_sender_id(), 80)
        ...
        yield event.plain_result(f"{label}的房间已经准备好了：\n{self._ticket_url(ticket)}\n链接会在短时间后失效。")

    def _room_base_url(self) -> str:
        return self.public_base_url or self.room_server.local_base_url

    def _ticket_url(self, ticket: RoomTicket) -> str:
        return (
            f"{self._room_base_url()}/?ticket={quote(ticket.token, safe='')}"
    ...
    def _get_provider_by_id(self, provider_id: str) -> Any | None:
        ...
        if provider is None:
            ...
            if isinstance(warned, set) and provider_id not in warned:
                ...
                logger.warning("[TogetherCompanion] 配置的 Provider 不存在或已下线，正在回退默认选择: id=%s", _single_line(provider_id, 80))
        return provider
    ...
    def _get_chat_provider(self) -> Any | None:
        ...
        try:
            ...
            return provider if provider is not None and hasattr(provider, "text_chat") else None
        except Exception:
            return None
    ...
    def _get_vision_provider(self) -> Any | None:
        ...
        add(self._get_provider_by_id(self.vision_provider_id))
        add(self._get_provider_by_id(self._astrbot_image_provider_id()))
        add(self._get_chat_provider())
        return next((provider for provider in candidates if self._provider_supports_image(provider)), None)
    ...
    async def handle_room_payload(self, room: RoomSession, payload: dict[str, Any]) -> None:
        ...
        if message_type == "user_text":
            ...
            if text:
                ...
                frame = self._normalize_frame_data_url(payload.get("frame")) if room.mode == "watch" else ""
        ...
        if message_type == "audio_utterance":
            ...
            if not audio_bytes:
                ...
                return
            self._start_room_task(room, self._transcribe_and_reply(room, audio_bytes, mime_type))
            return
    ...
    async def _generate_model_text(
        ...
        *,
        ...
        provider = self._get_vision_provider() if image_data_url else self._get_chat_provider()
        if provider is None:
            if image_data_url:
                raise RuntimeError("未配置支持图片输入的观影视觉模型")
    ...
    async def _resolve_astrbot_conversation(self, room: RoomSession) -> tuple[Any, str, str]:
    ...
    async def _transcribe_and_reply(self, room: RoomSession, audio_bytes: bytes, mime_type: str) -> None:
        ...
        try:
            ...
            if not text:
                ...
                return
            await self._correct_stt_and_reply(room, text[:4000], source="astrbot_stt")
        except asyncio.CancelledError:
            raise
        except Exception as exc:
